...
Configure logstash input like this
input {
  tcp {
    type => 'salt_grain_report'
    port => 5000
    codec => json
  }
}

In order to connect to a logstash Server, you must specify in the minion configuration the currently available server.
grainreport:
  ls_host: 'SERVERNAME'
  ls_port: '5000'
'''
...
def __virtual__():
  ...
  logstash_config = __opts__['grainreport'] if 'grainreport' in __opts__ else None

  if logstash_config:
    ls_host = logstash_config.get('ls_host', None)
    ls_port = logstash_config.get('ls_port', '5000')
...
def _get_logstash_configuration():
  '''
  Return the grainreport configuration read from the master config
  '''
  return {
     'ls_host': __opts__['grainreport']['ls_host'],
     'ls_port': __opts__['grainreport']['ls_port']
  }
...
def log_infos():
'''
  Log all Grains to a logstash server.
  Please configure the needed settings in the minion configuration. like this
...
'''
  ...
  tcpcon.close()
  #log.debug('ls_host: ' + ls_host + ' ls_port: ' + ls_port)
